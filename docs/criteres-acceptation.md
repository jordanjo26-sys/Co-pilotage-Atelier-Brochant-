...
| « Un bon de commande payé par Stripe est reconnu comme payé ; un bon de commande non payé peut bénéficier du délai de 30 jours » | ✅ Fait | `src/services/bonsCommande.ts` rapproche un paiement Stripe dont la description cite la référence du bon de commande (jamais de choix deviné si plusieurs paiements correspondent), sinon accorde un délai de grâce de 30 jours depuis l'émission (réutilise le mécanisme de délai accordé, section 4.3) |
