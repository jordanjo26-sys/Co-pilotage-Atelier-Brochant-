...
| « Un relevé de factures fournisseur est utilisé comme contrôle et non comme facture détaillée » | ⚠️ Partiel | Le relevé est reconnu et archivé séparément (`statutDext: archive`), jamais transmis à Dext. Premier niveau de contrôle croisé en place (`src/services/controleReleves.ts`) : signale une anomalie si un relevé arrive d'un fournisseur dont aucune facture n'est connue. Le contrôle ligne par ligne (chaque facture citée dans le relevé, avec son montant) reste à construire, faute d'exemple réel de relevé pour concevoir un rapprochement fiable (chaque fournisseur a sa propre mise en page PDF) |
| « Un payout Stripe groupé peut être ventilé jusqu'aux factures et rapproché du mouvement bancaire, frais inclus » | ✅ Fait | La ventilation payout → paiements (brut/frais/net) est faite (`GET /api/payouts/:payoutRef/ventilation`). Le rapprochement automatique payout ↔ mouvement bancaire (montant exact + date à 5 jours près, jamais de correspondance devinée en cas d'ambiguïté) est en place (`src/services/rapprochementBancaire.ts`), déclenché après chaque import bancaire/payout |
...
| « Un bon de commande payé par Stripe est reconnu comme payé ; un bon de commande non payé peut bénéficier du délai de 30 jours » | ✅ Fait (via l'import CSV) | En conditions réelles, c'est la **facture** qui est réglée par Stripe (pas un bon de commande séparé) : Synec enregistre le règlement avec une référence explicite (`Stripe pi_xxx`) dans la colonne `payments`, déjà reconnue par `parseReglements` et intégrée au statut de la facture à l'import. Un premier essai de rapprochement automatique dédié côté API Stripe, fondé sur une hypothèse de « bon de commande » cité en texte libre, s'est avéré ne pas correspondre à l'usage réel et a été retiré |
